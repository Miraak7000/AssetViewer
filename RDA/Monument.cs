...
namespace RDA {
  ...
  public static class Monument {
    ...
    private static void MonumentEventCategory() {
      ...
      document.Add(new XElement("MonumentEventCategory"));
      document.Root.Add(result.Select(s => s.ToXml()));
      document.Save($@"{Program.PathRoot}\Modified\Assets_MonumentEventCategory.xml");
      document.Save($@"{Program.PathViewer}\Resources\Assets\MonumentEventCategory.xml");
    }
    ...
    private static void MonumentEventReward() {
      ...
      var document = new XDocument();
      document.Add(new XElement("MonumentEventReward"));
      document.Root.Add(result.Select(s => s.ToXml()));
      document.Save($@"{Program.PathRoot}\Modified\Assets_MonumentEventReward.xml");
      document.Save($@"{Program.PathViewer}\Resources\Assets\MonumentEventReward.xml");
    }
    #endregion
