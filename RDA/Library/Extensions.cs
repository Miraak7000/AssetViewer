...
namespace RDA.Library {
  ...
  internal static class Extensions {
    ...
    public static void AddSourceAsset(this List<XElement> source, XElement element) {
      ...
      if (!source.Where(w => w.XPathSelectElement("Values/Standard/GUID").Value == assetID).Any() && !source.Where(w => w.XPathSelectElement("Values/Expedition/ExpeditionName")?.Value == expeditionName).Any()) {
        source.Add(element);
      }
    }
    #endregion
